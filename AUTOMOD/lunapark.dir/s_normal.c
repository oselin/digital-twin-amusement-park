...
static int32
am_S_normal(load* this, int32 step, void* args)
{
...
Label1: ;  /* Step1 */
	{
		{
			while (!(SecGetCurConts(ValidPtr(SysGetQualifier(am_model.am_conv.$sys, "sec_roller", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], -9999), 43, ConvSection*)) == 0)) { // wait until ...
				return waituntil(this, am_S_normal, Step 2, am_localargs, 0x00201040);
Label2: ; // Step 2
			}
		}
		{
			pushppa(this, am_S_normal, Step 3, am_localargs);
...
Label3: ; // Step 3
		}
		{
			if (am2_que[ValidIndex("am_model.am_que", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)] < QueGetCapacity(ValidPtr(&(am2_Q_roll[ValidIndex("am_model.am_Q_roll", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)]), 50, queue*)) - 1) {
				...
				{
					return waitorder(&(am2_OL_roller[ValidIndex("am_model.am_OL_roller", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)]), this, am_S_normal, Step 4, am_localargs);
Label4: ; // Step 4
				...
				{
					pushppa(this, am_S_normal, Step 5, am_localargs);
					pushppa(this, inqueue, Step 1, &(am2_Q_roll[ValidIndex("am_model.am_Q_roll", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)]));
					return Continue; // go move into territory
Label5: ; // Step 5
				}
...
/* init function for S_normal.m */
void
model_S_normal_init(struct model_struct* data)
{
	data->am_S_normal = am_S_normal;
}
