...
static int32
am_S_priority1(load* this, int32 step, void* args)
{
...
Label1: ;  /* Step1 */
	{
		{
			while (!(am2_numpriority[ValidIndex("am_model.am_numpriority", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)] < 10 || SecGetCurConts(ValidPtr(SysGetQualifier(am_model.am_conv.$sys, "sec_roller", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 0, -9999), 43, ConvSection*)) == 0)) { // wait until ...
				return waituntil(this, am_S_priority1, Step 2, am_localargs, 0x01201040);
Label2: ; // Step 2
			}
		}
		{
			pushppa(this, am_S_priority1, Step 3, am_localargs);
...
Label3: ; // Step 3
		}
		{
			if (am2_que[ValidIndex("am_model.am_que", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)] < QueGetCapacity(ValidPtr(&(am2_Q_roll[ValidIndex("am_model.am_Q_roll", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)]), 50, queue*)) - 1) {
				...
				{
					am2_numpriority[ValidIndex("am_model.am_numpriority", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)] += 1;
					EntityChanged(0x01000000);
				}
				{
					return waitorder(&(am2_OL_roller[ValidIndex("am_model.am_OL_roller", this->attribute->am2_roller[ValidIndex("am_model.am_roller", this->attribute->am2_cc, 10)], 5)]), this, am_S_priority1, Step 4, am_localargs);
Label4: ; // Step 4
...
LabelRet: ;
	if (am_localargs)
		xfree(am_localargs);
	return retval;
} /* end of am_S_priority1 */



/* init function for S_priority1.m */
void
model_S_priority1_init(struct model_struct* data)
{
	data->am_S_priority1 = am_S_priority1;
}
